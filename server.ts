...
async function startServer() {
  ...
  app.post('/api/generate', async (req, res) => {
    try {
      ...
      const prompt = `Act as a student innovation assistant for BloopLabs. 
Convert the following rough project idea into a structured, competition-ready project output. 
Use simple English. Keep it realistic and useful for school-level projects.
...
`;
      ...
      const responseSchema: Schema = {
        ...
        properties: {
          ...
          solution_summary: { type: Type.STRING, description: 'A concise summary of the proposed solution' },
          why_it_matters: { type: Type.STRING, description: 'Explanation of the project impact and importance' },
          uniqueness: { type: Type.STRING, description: 'What makes this solution unique or different' },
          feasibility: { type: Type.STRING, description: 'Notes on how feasible it is for a student to execute' },
          materials: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'List of materials needed' },
          steps: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'High-level steps to execute the project' },
          expected_outcome: { type: Type.STRING, description: 'What the expected final result or prototype is' },
          pitch: { type: Type.STRING, description: 'A short 2-3 sentence pitch for the project' },
          improvements: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'Suggestions to make the idea stronger' }
        },
        required: [
